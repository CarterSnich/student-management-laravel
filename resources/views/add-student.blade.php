    ...
    <form action="{{ url(request()->path() . '/submit') }}" class="bg-secondary-subtle p-5 rounded shadow position-relative" style="width: 600px" method="POST">
      ...
      <div class="row g-3">
        <div class="col-md-6">
          ...
          <input type="text" @class(['form-control', 'is-invalid' => $errors->has('firstname')]) id="firstname" name="firstname" value="{{ old('firstname') }}">
        </div>
        <div class="col-md-6">
          <label for="student_id" class="form-label">Student ID:</label>
          <input type="text" @class(['form-control', 'is-invalid' => $errors->has('student_id')]) id="student_id" name="student_id" value="{{ old('student_id') }}">
        </div>
        <div class="col-md-6">
          <label for="lastname" class="form-label">Last name:</label>
          <input type="text" @class(['form-control', 'is-invalid' => $errors->has('lastname')]) id="lastname" name="lastname" value="{{ old('lastname') }}">
        </div>
        <div class="col-md-6">
          <label for="birthdate" class="form-label">Birthdate:</label>
          <input type="date" @class(['form-control', 'is-invalid' => $errors->has('birthdate')]) id="birthdate" name="birthdate" value="{{ old('birthdate') }}">
        </div>
        <div class="col-md-6">
